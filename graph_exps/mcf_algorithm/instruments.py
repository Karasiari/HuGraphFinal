...
def _shortest_path_with_edge_keys(G, source, target, edge_costs):
...
def _copy_and_filter_graph(flow_graph, demand_capacity):
...
def _group_demands_and_create_mapping(demands, unsatisfied_demands: set):
    grouped_demands = []
...
def _shortest_path_with_l(G, source, sink):
...
def _update_l_on_path(G, path, eps):
    for u, v, key in path:
        G[u][v][key]['l'] *= (1 + eps)


# Main flow procedure
def _multi_commodity_flow(G, grouped_demands, C_max, eps=0.1):
    # Initialize l(e)
    _initialize_l(G, C_max, eps)
    ...
    iter_num = 0
    while _D(G, C_max) < 1 and iter_num < iter_max:
        iter_num += 1
        for i, demand in enumerate(grouped_demands):
            ...
            while _D(G, C_max) < 1 and d_i > 0:
                # Find the shortest path based on current l(e)
                path = _shortest_path_with_l(G, source, sink)
                ...
                # Update l(e) -> l(e) * (1 + eps) for all edges in the path
                _update_l_on_path(G, path, eps)

    return flow


# Function to scale the flow to make it feasible
def _scale_flows(flow, G, C_max):
...
def _subtract_flow_from_graph(flow_graph, path, demand_capacity):
    path_with_keys = []
...
def _subdivide_flows_by_paths(flow, demand_indices_by_group, ungrouped_demands, i_to_source_target):
    ...
    for i, demand_indices in demand_indices_by_group.items():
        ...
        for demand_index in sorted_demand_indices:
            ...
            filtered_graph = _copy_and_filter_graph(flow_graph, demand.capacity)
            ...
            try:
                ...
                path_with_keys = _subtract_flow_from_graph(flow_graph, path, demand.capacity)
...
def _fulfill_remaining_demands(graph_copy, ungrouped_demands, demand_indices_by_group, i_to_source_target,
                              left_to_satisfy: set):
    remaining_paths = {}
    satisfied_demands = []
    ...
    for i, demand_indices in demand_indices_by_group.items():
        ...
        for demand_index in sorted_demand_indices:
            ...
            if demand_index not in left_to_satisfy:
                continue
            # Copy and filter the graph for edges that can handle the demand capacity
            filtered_graph = _copy_and_filter_graph(graph_copy, demand.capacity)
            if not source in filtered_graph:
                continue
            if not sink in filtered_graph:
                continue
            try:
                ...
                path_with_keys = _subtract_flow_from_graph(graph_copy, path, demand.capacity)
